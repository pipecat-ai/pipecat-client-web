...
describe("BotOutput assembly", () => {
  ...
  describe("S2S race: spoken events arrive before unspoken", () => {
    ...
    it("absorbs spoken fragments that contain TTS sub-word splits (Pi/pec/at)", () => {
      ...
      expect(cursor.currentPartIndex).toBe(0);
      const spokenPrefix = "Let me check the Pipecat documentation for details ";
  ...
  // -----------------------------------------------------------------------
  // Cascaded pipeline (unspoken first): absorption + drop logic inert
  // -----------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------
  // TTS mid-sentence tokenization + cursor drift regression
  // -----------------------------------------------------------------------
  describe("TTS splits mid-sentence tokens after unspoken arrived", () => {
    it("advances cursor through a sentence with contractions and sub-word splits", () => {
      // Regression: cursor used to drift one word behind for every spoken
      // event that had to skip past earlier words, compounded by contractions
      // over-counting word starts. After the fix, cursor reaches end of
      // sentence even with "Pi"/"pec"/"at" sub-word splits.
      harness.ensureAssistantMessage();
    ...
    it("keeps unmatched spoken words as fallbacks when the current sentence is fully consumed", () => {
      // After sentence 1 finishes, TTS begins speaking sentence 2 before the
      // unspoken aggregator emits. Those words must survive as fallbacks.
      harness.ensureAssistantMessage();
    ...
    it("handles multi-word spoken tokens that cross a sentence boundary", () => {
      ...
      // are also emitted. All of these should be split and processed
  ...
  // -----------------------------------------------------------------------
  // Non-spoken aggregation types (skip_tts-style content)
  // -----------------------------------------------------------------------
