...
Both packages ship a v1 client implementation of the UI Worker Protocol, a structured wire format that lets server-side workers observe and drive a GUI app. Pairs with the `UIWorker` class in [Pipecat](https://github.com/pipecat-ai/pipecat) on the Python side.

The client side provides:

- **`PipecatClient`** (`client-js`): sends `ui-event` messages, exposes `RTVIEvent.UICommand` / `RTVIEvent.UIJobGroup` subscriptions and matching constructor callbacks, manages accessibility snapshot streaming with `startUISnapshotStream(...)`, and can cancel UI job groups.
- **`A11ySnapshotStreamer`** (`client-js`): low-level snapshot streaming implementation for advanced users.
- **React bindings** (`client-react`): `UIJobGroupsProvider`, hooks (`useUIEventSender`, `useUICommandHandler`, `useUIJobGroups`, `useUISnapshot`), and opt-in DOM defaults (`useDefaultScrollToHandler`, `useDefaultHighlightHandler`, `useDefaultSelectTextHandler`, `useDefaultSetInputValueHandler`, `useDefaultClickHandler`, `useDefaultFocusHandler`).
