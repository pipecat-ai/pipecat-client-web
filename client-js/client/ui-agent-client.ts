 ...
 * start listening for `RTVIEvent.UICommand` and `RTVIEvent.UITask`,
 * and store the returned detach function to stop. The React provider
 * does this automatically inside a `useEffect` so subscription
 * lifecycle follows mount/unmount.
 */
export class UIAgentClient {
   ...
   *
   * Sends a first-class `ui-cancel-task` RTVI message the server's
   * `UIAgent` routes to `cancel_task`. The server honors the request
   * only when the group was registered with `cancellable: true`;
   * otherwise the request is silently ignored.
   *
  ...
  /**
   * Subscribe to `RTVIEvent.UICommand` and `RTVIEvent.UITask` on the
   * underlying `PipecatClient`. Returns a detach function that
   * unsubscribes both.
   *
   ...
   */
  attach(): () => void {
    const commandListener = (data: unknown) => this._handleUICommand(data);
    const taskListener = (data: unknown) => this._handleUITask(data);
    this._client.on(RTVIEvent.UICommand, commandListener);
    this._client.on(RTVIEvent.UITask, taskListener);
    return () => {
      this._client.off(RTVIEvent.UICommand, commandListener);
      this._client.off(RTVIEvent.UITask, taskListener);
    };
  }
  ...
  private _handleUITask(data: unknown): void {
    if (!data || typeof data !== "object") return;
